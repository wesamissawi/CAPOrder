...
async function fetchOrderDetailsFromList(page, reference) {
  ...
  // Wait for detail rows to appear in the body frame (frame swaps can be async)
  try {
    const start = Date.now();
    const timeoutMs = 20000;
    while (Date.now() - start < timeoutMs) {
      try {
        await page.waitForTimeout(300); // give frameset a moment to swap
        bodyFrame = await getBodyFrame(page, 4000);
        await bodyFrame.waitForSelector("tr.PartO, tr.PartE", { timeout: 1200 });
        break;
      } catch (_) {
        // keep looping until timeout
      }
    }
    await bodyFrame.waitForSelector("tr.PartO, tr.PartE", { timeout: 1000 });
  } catch (err) {
